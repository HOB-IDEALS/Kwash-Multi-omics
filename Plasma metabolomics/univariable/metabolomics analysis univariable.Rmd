---
title: "Plasma metabolome univariable analysis"
...
This is the unvariable analysis of the plasma metabolomics data

```{r cars}
prot <- read.csv("MSCombine_plasma_metabolomics.csv", header = TRUE, sep = ",")
head(prot[,1:15])
```

Perform kwash vs others analysis using conditional logistic regression at admission to determine which metabolites are interesting
...
GLMs.out <- lapply(colnames(admission[,c(11:2835)]),GLM.run )
...
rownames(pVal) = colnames(admission[11:2835])
fdr = as.matrix(p.adjust(as.vector(results), method ="fdr", n=2825)) #n=number of metabolites
pVal=cbind(pVal, fdr)
...
rownames(estimate) = colnames(admission[11:2835])
...
rownames(stderror) = colnames(admission[11:2835])
...
round2 <- cbind(kadmission[, 1:10], kadmission[,round2])
View(round2)
...
GLMs.out <- lapply(colnames(round2[,c(11:70)]),GLM.run )
results<-lapply(GLMs.out, function(x){summary(x)})
#results
...
polr$fdr = p.adjust(polr$p_value, method ="fdr", n=60)
polr
rownames(polr) = colnames(admission[11:70])
...
# Perform PCA
pca_result <- prcomp(prot[,-(1:10)], center = TRUE, scale. = TRUE)
